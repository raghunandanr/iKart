...
static char* DURATION_KEY = "DURATION";
...
- (void)startGlowingWithColor:(UIColor *)color intensity:(CGFloat)intensity withDuration: (NSNumber*) duration{
    ...
    [self setDurationBtw1:duration];
    [self startGlowingWithColor:color fromIntensity:0.1 toIntensity:intensity repeat:YES];
}

- (void)glowOnceWithColor:(UIColor *)color intensity:(CGFloat)intensity
{
    [self startGlowingWithColor:color fromIntensity:0.1 toIntensity:intensity repeat:NO];
}
...
- (void) setDurationBtw1:(NSNumber*)durationBtw
{
    objc_setAssociatedObject(self, DURATION_KEY, durationBtw, OBJC_ASSOCIATION_RETAIN);
...
{
    return objc_getAssociatedObject(self, DURATION_KEY);

}

- (NSNumber*) durationBtw1
{
    return objc_getAssociatedObject(self, DURATION_KEY);
...
- (void) startGlowingWithColor:(UIColor*)color fromIntensity:(CGFloat)fromIntensity toIntensity:(CGFloat)toIntensity repeat:(BOOL)repeat {
    ...
    if(![self durationBtw])
    {
        [self setDurationBtw1:[NSNumber numberWithFloat:DUR_BTW]];
    }
    animation.duration = [[self durationBtw1] floatValue];
...
{
    [self startGlowingWithColor:[UIColor yellowColor] intensity:0.6];

}
- (void) glowOnceAndKillAfterSecs: (int64_t)dur
{
    [self startGlowingWithColor:[UIColor yellowColor] intensity:0.6];
    dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, dur * NSEC_PER_SEC);
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        [self stopGlowing];
    });
