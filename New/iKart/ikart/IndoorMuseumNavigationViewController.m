...
- (void)viewDidLoad {
  ...
  GMSCameraPosition *camera = [GMSCameraPosition cameraWithLatitude:37.179758
                                                          longitude:-77.371352
                                                               zoom:17];
...
- (void)addMarkerForPolygonAtCoordinate:(CLLocationCoordinate2D)iLocation WithTitle:(NSString *)iTitle withMessage:(NSString *)iMessage withMarkerObject:(id)iMarkerObject
{
    GMSMarker *marker = [GMSMarker markerWithPosition:iLocation];
    marker.position = iLocation;
    marker.title = iTitle;
    marker.snippet = iMessage;
    marker.userData = iMarkerObject;
    marker.infoWindowAnchor = CGPointMake(1.2, -0.11);
    marker.map = mapView_;
}
